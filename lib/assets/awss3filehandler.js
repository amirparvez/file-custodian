...
class FCAWSS3FileHandler extends FileHandler{
            ...
            if(validateConfig.object.request){
                ...
                for(let requestFile of requestFiles){
                    ...
                    if(contents){
                        ...
                        let newS3FileCreated = await this.makeS3File(`${self.config.base_path}${self.config.base_path === "" ? "" : "/"}${fileFolder}${fileFolder === "" ? "" : "/"}${requestFileName}`, finalContents, doEncrypt, isStream, contentLength);
                ...
                let newS3FileCreated = await this.makeS3File(`${self.config.base_path}${self.config.base_path === "" ? "" : "/"}${fileFolder}${fileFolder === "" ? "" : "/"}${validateConfig.object.name}.${validateConfig.object.ext}`, finalContents, doEncrypt, validateConfig.object.isStream, validate.object.contentLength);
                if(newS3FileCreated === true){
                    if(doEncrypt === true){ didEncrypt = true; }
    ...
    async searchFiles(options){
        ...
        if(validateConfig.success){
            ...
            let files = await this.searchS3Files(pathToFile.toString().trim() === "*" ? "*" : `${self.config.base_path}${self.config.base_path === "" ? "" : "/"}${pathToFile}`, validateConfig.object.query, validateConfig.object.forceRequestToS3);
            if(files){
                let FCFiles = [];
    ...
    async syncDatabase(){
        try{
            let files = await this.getAllFilesOfS3Directory(this.config.base_path, true);
    ...
    async replaceFileContents(options){
        ...
        try{
            if(validateConfig.success){
                ...
                return await this.makeS3File(`${this.config.base_path}${this.config.base_path === "" ? "" : "/"}${filePath}`, validateConfig.object.new_contents, validateConfig.object.doEncrypt, validateConfig.object.isStream, validateConfig.object.contentLength);
            }else{ return false; }
        }catch(error){
            return false;
        }
    ...
    async getFileContents(file){
        try{
            ...
            return await this.getS3FileContents(`${this.config.base_path}${this.config.base_path === "" ? "" : "/"}${filePath}`, file.config.isEncrypted);
        }catch(error){
