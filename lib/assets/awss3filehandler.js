...
class FCAWSS3FileHandler extends FileHandler{
    ...
    async syncDatabase(){
        try{
            let files = await this.getAllFilesOfS3Directory(this.config.basePath, true);
    ...
    async getFileContents(file){
        try{
            ...
            return await this.getS3FileContents(`${this.config.basePath}${this.config.basePath === "" ? "" : "/"}${filePath}`, file.config.isEncrypted);
        }catch(error){
