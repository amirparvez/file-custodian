        ...
        if(validateConfig.success){
            if(validateConfig.object.request){
                ...
                for(let requestFile of requestFiles){
                    ...
                    let contentSize = requestFile.size;
                    ...
                    if(contents){
                        ...
                        let newS3FileCreated = await this.makeS3File(`${self.config.base_path}${self.config.base_path === "" ? "" : "/"}${fileFolder}${fileFolder === "" ? "" : "/"}${requestFileName}`, finalContents, doEncrypt, isStream, contentSize);
                ...
                let newS3FileCreated = await this.makeS3File(`${self.config.base_path}${self.config.base_path === "" ? "" : "/"}${fileFolder}${fileFolder === "" ? "" : "/"}${validateConfig.object.name}.${validateConfig.object.ext}`, finalContents, doEncrypt, validateConfig.object.isStream, validate.object.contentSize);
                if(newS3FileCreated === true){
                    if(doEncrypt === true){ didEncrypt = true; }
    ...
    async replaceFileContents(options){
        ...
        try{
            if(validateConfig.success){
                ...
                return await this.makeS3File(`${this.config.base_path}${this.config.base_path === "" ? "" : "/"}${filePath}`, validateConfig.object.new_contents, validateConfig.object.doEncrypt, validateConfig.object.isStream, validate.object.contentSize);
            }else{ return false; }
        }catch(error){
            return false;
        }
