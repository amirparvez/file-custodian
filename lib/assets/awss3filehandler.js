...
class FCAWSS3FileHandler extends FileHandler{
            ...
            if(validateConfig.object.request){
                ...
                for(let requestFile of requestFiles){
                    ...
                    if(contents){
                        ...
                        let newS3FileCreated = await this.makeS3File(`${self.config.base_path}${self.config.base_path === "" ? "" : "/"}${fileFolder}${fileFolder === "" ? "" : "/"}${requestFileName}`, finalContents, readStream, doEncrypt, isStream, contentLength, false);
    ...
    async replaceFileContents(options){
        ...
        try{
            if(validateConfig.success){
                ...
                return await this.makeS3File(`${this.config.base_path}${this.config.base_path === "" ? "" : "/"}${filePath}`, validateConfig.object.new_contents, validateConfig.object.readStream, validateConfig.object.doEncrypt, validateConfig.object.isStream, validateConfig.object.contentLength, true);
            }else{ return false; }
        }catch(error){
            return false;
        }
    ...
    async makeS3File(path, contents, readStream, doEncrypt, isStream, contentLength, isReplacing = false){
        ...
        try{
            ...
            if(doEncrypt === true){
                const encryptStream = self.createEncryptStream(self.config.encryptingSpeed, contentStream, contentLength);
