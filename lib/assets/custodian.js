...
class FCCustodian{
        ...
        if(options && typeof options === "object" && typeof options.name === "string" && typeof options.isDefault === "boolean"){
            ...
            if(validateConfig.object.type == "local-server"){
                await this.file_handlers.push({
                    ...
                    handler: new FCLocalServerFileHandler({ type: validateConfig.object.type, base_path: validateConfig.object.base_path, encryptingSpeed: validateConfig.object.encryptingSpeed, readingSpeed: validateConfig.object.readingSpeed, writingSpeed: validateConfig.object.writingSpeed, }),
                });
            }else{
                if(validateConfig.object.type == "aws-s3"){
                    await this.file_handlers.push({
                        name: options.name,
                        isDefault: options.isDefault,
                        handler: new FCAWSS3FileHandler({ type: validateConfig.object.type, base_path: validateConfig.object.base_path, bucket_name: validateConfig.object.bucket_name, bucket_region: validateConfig.object.bucket_region, key: validateConfig.object.key, key_id: validateConfig.object.key_id, encryptingSpeed: validateConfig.object.encryptingSpeed, readingSpeed: validateConfig.object.readingSpeed, writingSpeed: validateConfig.object.writingSpeed, }),
                    });
                }
            }
    ...
    async user(user_id){
        const defaultFH = this.file_handlers.find(x => {return x.isDefault === true});
        if(defaultFH && defaultFH.handler){
            return defaultFH.handler.user(user_id);
        }else{ if(Config.debug){ throw Errors.find(x => {return x.abbr == "c_idfh"}).errorObj; }else{ return null; } }
    }
