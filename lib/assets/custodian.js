...
class FCCustodian{
    ...
    // Calls syncDB function of default file handler.
    async syncDB(){
        const defaultFH = this.file_handlers.find(x => {return x.isDefault === true});
        if(defaultFH && defaultFH.handler){
            let files = await defaultFH.handler.syncDB();
            return files;
        }else{ if(Config.debug){ throw Errors.find(x => {return x.abbr == "c_idfh"}).errorObj; }else{ return []; } }
    }

    // Private function.
    // Calls replaceFileData function of default file handler.
    async replaceFileData(options){
        const defaultFH = this.file_handlers.find(x => {return x.isDefault === true});
        if(defaultFH && defaultFH.handler){
            let response = await defaultFH.handler.replaceFileData(options);
            return response;
        }else{ if(Config.debug){ throw Errors.find(x => {return x.abbr == "c_idfh"}).errorObj; }else{ return false; } }
    }
}
