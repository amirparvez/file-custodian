...
class FCMongoDBHandler{
    ...
    async createTable(){
        try{
            ...
            await this.FileModel.collection.drop();
            Logger.log("Table created");
        }catch(error){
            Logger.log(error);
            return true; // Collection does not exists, therefore can not be dropped.
        }
    ...
    async getModel(options){
        try{
            if(typeof options === "object"){
                ...
                if(options.id){
                    if(options.userId){
                        model = await this.FileModel.findOne({ _id: options.id, userId: options.userId, isDeleted: false, });
                    }else{
                        model = await this.FileModel.findOne({ _id: options.id, isDeleted: false, });
                    }
                }else{
                    if(options.path){
                        if(options.userId){
                    ...
                    if(validateConfig.object.isEncrypted !== undefined && validateConfig.object.isEncrypted !== null){
                        ...
                        if(validateConfig.object.size !== undefined && validateConfig.object.size !== null){
                            ...
                            if(validateConfig.object.newPath){
                                ...
                                updates = { folder: folderFromPath, path: newFilePath };
                            }
                        }
                    }
                }
