...
class FCMongoDBHandler{
    ...
    async getModel(options){
        if(typeof options === "object"){
            ...
            if(options.id){
                if(options.userId){
                    model = await this.FileModel.findOne({ id: options.id, userId: options.userId, isDeleted: false, });
                }else{
                    model = await this.FileModel.findOne({ id: options.id, isDeleted: false, });
                }
            }else{
                if(options.path){
                    if(options.userId){
