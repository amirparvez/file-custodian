...
class FCMySQLHandler{
        ...
        if(validateConfig.success){
            ...
            if(this.config.sequelizeInstance !== null){
                ...
                this.sequelize = new Sequelize(this.config.database, this.config.userName, this.config.password, {
                    ...
                    timezone: "+00:00",
                    logging: process.env.FILECUSTODIAN_DEBUG && process.env.FILECUSTODIAN_DEBUG.toString().trim() === "true" ? true : false,
                    dialectOptions: {
    ...
    async getModel(options){
        if(typeof options === "object"){
            if(options.id){
                if(options.userId){
                    return await this.FileModel.findOne({ where: { id: options.id, userId: options.userId, isDeleted: false, }, include: {all: true,}, });
                }else{
                    return await this.FileModel.findOne({ where: { id: options.id, isDeleted: false, }, include: {all: true,}, });
                }
            }else{
                if(options.path){
                    if(options.userId){
                        return await this.FileModel.findOne({ where: { path: options.path, userId: options.userId, isDeleted: false, }, include: {all: true,}, });
                    }else{
                        return await this.FileModel.findOne({ where: { path: options.path, isDeleted: false, }, include: {all: true,}, });
                    }
                }
            }
        }
                ...
                if(validateConfig.object.newName){
                    ...
                    if(validateConfig.object.isEncrypted !== undefined && validateConfig.object.isEncrypted !== null){
                        ...
                        if(validateConfig.object.size !== undefined && validateConfig.object.size !== null){
                            await model.update({ size: validateConfig.object.size });
                        }else{
                            if(validateConfig.object.newPath){
                                const folderFromPath = path.dirname(validateConfig.object.newPath);
                                const newFilePath = `${folderFromPath}${folderFromPath === "" ? "" : "/"}${model.name}.${model.extension}`;
                                await model.update({ folder: folderFromPath, path: newFilePath });
                            }
                        }
                    }
                }
