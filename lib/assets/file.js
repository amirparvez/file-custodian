...
class FCFile{
    ...
    // Copies this file to another depository and update the database.
    async copyToDepository(depository){
        if(!(depository instanceof FileHandler) || depository.config.name === this.config.handler.config.name){ return null; };

        let doesFileAtNewPathAlreadyExists = await depository.getFile({ name: this.config.name, ext: this.config.ext, folder: this.config.folder }); // This will log a NOTFOUND error
        if(doesFileAtNewPathAlreadyExists === null){
            const { contents, contentType, contentLength, readStream } = await this.getContents();
            return await depository.newFile({
                name: this.config.name,
                ext: this.config.ext,
                folder: this.config.folder,
                isStream: true,
                isEncrypted: this.config.isEncrypted === true ? false : true, // Passing isEncrypted = false to newFile() encrypts the contents but with true it ignores encryption.
                contentLength: this.config.isEncrypted === true ? contentLength-34 : contentLength,
                contents: contents,
                readStream: readStream, // Passing readStream is important here since contents can be a pipeline which causes pause/resume problems in make<depositoryType>File() function.
            });
        }
    ...
    // Moves this file and update the database.
    async moveToFolder(folderName){
        if(folderName === this.config.folder){ return null; };
        let doesFileAtNewPathAlreadyExists = await this.config.handler.getFile({ name: this.config.name, ext: this.config.ext, folder: folderName }); // This will log a NOTFOUND error
        if(doesFileAtNewPathAlreadyExists === null){
            return await this.config.handler.moveFile({ name: this.config.name, ext: this.config.ext, folder: this.config.folder, isEncrypted: this.config.isEncrypted, folderName });
        }

        return null;
    }

    // Moves this file to another depository and update the database.
    async moveToDepository(depository){
        if(!(depository instanceof FileHandler) || depository.config.name === this.config.handler.config.name){ return null; };

        const copiedFile = await this.copyToDepository(depository);
        if(copiedFile instanceof FCFile){
            const didDelete = await this.delete();
            return copiedFile;
        }
