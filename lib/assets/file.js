...
class FCFile{
    ...
    // If file contents is not encrypted according the database entry, encrypt it.
        ...
        try{
            if(this.config.isEncrypted == false && this.config.handler.file_protector !== null){
                ...
                if(doEncrypt == true){
                    ...
                    let { contents, contentType, readStream } = await this.getContents();

                    // Replace file contents with encrypted contents.
                    didEncrypt = await this.config.handler.replaceFileContents({ file: this, new_contents: contents, doEncrypt: true, isStream: true, });
