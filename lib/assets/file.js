...
class FCFile{
    ...
    async init(){
        ...
        if(validateConfig.success){
            ...
            let iv = data.isEncrypted === true ? await data.handler.readFileIV(finalFileFullPath) : null;
            ...
            if(this.config.handler.database_handler){
                try{
                    ...
                    if(model){
                        ...
                        iv = model.isEncrypted === true ? await data.handler.readFileIV(finalFileFullPath) : null;
    ...
    async rename(newName){
        ...
        let isFileWithNewNameAlreadyExists = await this.config.handler.getFile({ name: newName, ext: this.config.ext, folder: this.config.folder });
        if(isFileWithNewNameAlreadyExists === null){
            ...
            if(this.config.isEncrypted == false && this.config.handler.file_protector !== null){
                ...
                if(doEncrypt == true){
                    ...
                    if(didEncrypt == true){
                        ...
                        try{
                            // Update IV.
                            const folderPath = await this.config.handler.beautifyPath(this.folder, false);
                            const beautifiedPath = await this.config.handler.beautifyPath(folderPath, true); // Full folder path
                            const finalFileFullPath = `${beautifiedPath}${beautifiedPath == "" ? "" : "/"}${this.name}.${this.ext}`; // Full file path

                            let iv = await this.config.handler.readFileIV(finalFileFullPath);
                            this.config = {...this.config, iv };
                        }catch(error){ Logger.log(error); }
                    }
            ...
            if(this.config.isEncrypted == true && this.config.handler.file_protector !== null){
                ...
                if(doDecrypt == true){
                    ...
                    if(didDecrypt == true){
                        ...
                        try{
                            // Update IV.
                            this.config = {...this.config, iv: null, };
                        }catch(error){ Logger.log(error); }
                    }
