            ...
            if(this.config.isEncrypted == false && this.config.handler.file_protector !== null){
                ...
                if(doEncrypt == true){
                    ...
                    if(didEncrypt == true){
                        ...
                        try{
                            var response = await this.config.handler.replaceFileData({ file: this, new_data: newData, });
    ...
    async createModel(){
        ...
        if(this.config.handler.database_handler){
            ...
            if(alreadyExists !== null && alreadyExists !== undefined){
                ...
                await this.protect();
                var newModel = await this.config.handler.database_handler.newModel({...this.config, user_id: this.config.handler.config.user_id, path: filePath, });
