...
class FCFileHandler{
    ...
    async makeLocalFile(path, data, doEncrypt, isStream){
        let self = this;
        if(isStream === false){
            // Data is not a read stream. doEncrypt plays no role here.
            ...
            if(isStream === true){
                // Data is a read stream. doEncrypt plays a role here.
                try{
                    if(doEncrypt === true){
    ...
    async getLocalFile(fpath){
        ...
        if(isValid){
            ...
            if(isFile){
                ...
                const obj = { name: parsedPath.name, ext: parsedPath.ext.replace(".", ""), folder: folderPath == "" ? "/" : folderPath, };
    ...
    async getContentType(fpath){
        const self = this;
        const isValid = await self.doesLocalPathExists(fpath);
        if(isValid){
            const isFile = await self.isLocalPathOfFile(fpath);
            if(isFile){
    ...
    async getLocalFileContents(path, doDecrypt){
        ...
        if(isValid){
            ...
            if(isFile){
                const size = await self.getLocalFileSize(path);
                if(size <= /*5242880*/0){
                    // If file size is less than or equal to 5MB, read all at once.
                    const data = await fs.promises.readFile(path).then(async contents => {
                        return contents;
                    }).catch(error => {
                        return { contents: null, contentType: null, isStream: false, readStream: null, };
                    });

                    let decryptedData = null;
                    if(doDecrypt === true && self.file_protector !== null){ decryptedData = await self.file_protector.read(data); }

                    return { contents: decryptedData !== null ? decryptedData : data, contentType: contentType, isStream: false, readStream: null, };
                }else{
                    // If file size is greater than 5MB, read with stream.
                    try{
                        ...
                        if(doDecrypt === true && self.file_protector !== null){
                            ...
                            const pipeline = pipelineWithoutPromise(readStream, decrypt, (error) => { console.log("ERROR: ", error); });
                            readStream.pause();

                            return { contents: pipeline, contentType: contentType, isStream: true, readStream: readStream, };
                        }else{
                            return { contents: readStream, contentType: contentType, isStream: true, readStream: readStream, };
                        }
                    }catch(error){ return { contents: null, contentType: null, isStream: false, readStream: null, }; }
                }
            }
        }
