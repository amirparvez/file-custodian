...
class FCFileHandler{
    ...
    async makeLocalFile(path, contents, doEncrypt, isStream){
        ...
        try{
            ...
            if(doEncrypt === true){
                let encrypt = new stream.Transform({ readableHighWaterMark: self.config.readingSpeed, decodeStrings: false });
                ...
                readStream.resume();
                await pipelineWithPromise(readStream, encrypt, fs.createWriteStream(path, {flags: 'a', highWaterMark: self.config.writingSpeed, }));
            }else{
                readStream.resume();
    ...
    async makeS3File(path, contents, doEncrypt, isStream, contentSize){
        const self = this;
        const contentType = await this.getContentType(path);

        try{
            const speed = this.config.encryptingSpeed;

            var readStream = contents;
            if(isStream === false){
                readStream = stream.Readable.from(contents.toString(), { highWaterMark: speed, });
                readStream.pause();
            }
            ...
            if(doEncrypt === true){
                ...
                var processingProgress = `0%, (${bytesProcessed}/${contentSize})`;
                var encryptingProgress = `0%, (${bytesEncrypted}/${contentSize})`;
                var chunkCollection = Buffer.from("");

                let encrypt = new stream.Transform({ readableHighWaterMark: speed, writeableHighWaterMark: speed, decodeStrings: false });

                encrypt._transform = async function(chunk, encoding, done) {
                    let doThrottle = false;
                    readStream.pause();

                    console.log(chunk.length, speed, contentSize);
                    if(chunk.length === speed){
                        bytesProcessed = bytesProcessed + chunk.length;
                        processingProgress = `${(bytesProcessed/contentSize)*100}%, (${bytesProcessed}/${contentSize})`;
                        console.log(`[Processing] ${processingProgress}`)

                        let encrypted = await self.file_protector.protect(chunk);

                        readStream.resume();

                        bytesEncrypted = bytesEncrypted + chunk.length;
                        encryptingProgress = `${(bytesEncrypted/contentSize)*100}%, (${bytesEncrypted}/${contentSize})`;
                        console.log(`[Encrypting] ${encryptingProgress}`)

                        done(null, encrypted);
                    }else{
                        chunkCollection = Buffer.concat([chunkCollection, chunk]);
                        doThrottle = true;
                    }
                    ...
                    if(doThrottle === true){
                        ...
                        console.log(`Chunk collection length: ${chunkCollection.length}`);
                        while(bytesProcessed !== contentSize){
                            let bytesBeingProcessed = chunkCollection.slice(0, speed);
                            if(bytesBeingProcessed.length < speed && bytesBeingProcessed.length !== 0){
                                if((contentSize - bytesProcessed) <= speed){
                                    console.log(`Last chunk to be processed ${chunksInQueue.length+1},${bytesBeingProcessed.length}`);
                                }else{
                                    console.log(`Chunk IGNORED ${chunksInQueue.length+1},${bytesBeingProcessed.length}`);
                                    break;
                                }
                            }

                            chunksInQueue.push(bytesBeingProcessed);
                            //console.log(`Chunk added to queue ${chunksInQueue.length+1}`);

                            chunkCollection = chunkCollection.slice(speed, chunkCollection.length);
                            //console.log(`Chunk collection length: ${chunkCollection.length}`);
                        ...
                        console.log(`[Result] Total chunks in queue ${chunksInQueue.length}: `);

                        for(let chunkToEncrypt of chunksInQueue){
                            let encryptedChunk = await self.file_protector.protect(chunkToEncrypt);
                            encryptedChunks.push(encryptedChunk);
                            //console.log(`Chunk encrypted ${encryptedChunks.length+1}`, chunkToEncrypt.length, encryptedChunk.length);

                            bytesEncrypted = bytesEncrypted + encryptedChunk.length;
                            encryptingProgress = `${(bytesEncrypted/contentSize)*100}%, (${bytesEncrypted}/${contentSize})`;
                            console.log(`[Encrypting] ${encryptingProgress}`)
                        }

                        console.log(`[Result] Total chunks encrypted ${encryptedChunks.length}/${chunksInQueue.length}: `);
                ...
                const pipeline = pipelineWithoutPromise(readStream, encrypt, (error) => { });
            ...
            return true;
        }catch(error){ console.log(error); Config.debug ? console.log(error) : false; return false; }
        return false;
    }
    ...
    async getS3FileContents(path, doDecrypt){
        ...
        if(isValid){
            ...
            try{
                const readStream = this.config.s3.getObject({ Bucket: this.config.bucket_name, Key: path, }).createReadStream();
                readStream.pause();

                const speed = this.config.encryptingSpeed;
                const fileSize = await this.getS3FileSize(path);

                var bytesProcessed = 0;
                var bytesDecrypted = 0;
                var processingProgress = `0%, (${bytesProcessed}/${fileSize})`;
                var decryptingProgress = `0%, (${bytesDecrypted}/${fileSize})`;
                var chunkCollection = Buffer.from("");

                if(doDecrypt === true && self.file_protector !== null){
                    let decrypt = new stream.Transform({ readableHighWaterMark: speed, writeableHighWaterMark: speed, decodeStrings: false });
                    decrypt._transform = async function(chunk, encoding, done) {
                        ...
                        if(chunk.length === speed){
                            bytesProcessed = bytesProcessed + chunk.length;
                            processingProgress = `${(bytesProcessed/fileSize)*100}%, (${bytesProcessed}/${fileSize})`;
                            console.log(`[Processing] ${processingProgress}`)

                            let decrypted = await self.file_protector.read(chunk);
                            readStream.resume();

                            bytesDecrypted = bytesDecrypted + chunk.length;
                            decryptingProgress = `${(bytesDecrypted/fileSize)*100}%, (${bytesDecrypted}/${fileSize})`;
                            console.log(`[Decrypting] ${decryptingProgress}`)
                        ...
                        if(doThrottle === true){
                            ...
                            while(bytesProcessed !== fileSize){
                                let bytesBeingProcessed = chunkCollection.slice(0, speed);
                                if(bytesBeingProcessed.length < speed && bytesBeingProcessed.length !== 0){
                                    //console.log((fileSize - bytesProcessed));
                                    if((fileSize - bytesProcessed) <= speed){
                                        console.log(`Last chunk to be processed ${chunksInQueue.length+1},${bytesBeingProcessed.length}`);
                                    }else{
                                        console.log(`Chunk IGNORED ${chunksInQueue.length+1},${bytesBeingProcessed.length}`); // Next round
                                        break;
                                    }
                                }
                            ...
                            for(let chunkToDecrypt of chunksInQueue){
                                ...
                                console.log(`Chunk decrypted ${decryptedChunks.length+1}`, chunkToDecrypt.length, decryptedChunk.length);

                                bytesDecrypted = bytesDecrypted + decryptedChunk.length;
                                decryptingProgress = `${(bytesDecrypted/fileSize)*100}%, (${bytesDecrypted}/${fileSize})`;
                                console.log(`[Decrypting] ${decryptingProgress}`)
                            }

                            console.log(`[Result] Total chunks decrypted ${decryptedChunks.length}/${chunksInQueue.length}: `);
            ...
            }catch(error){ console.log(error); return { contents: null, contentType: null, readStream: null, }; }
        }
        ...
        if(isValid){
            ...
            if(isFile){
                try{
                    ...
                    if(doDecrypt === true && self.file_protector !== null){
                        let decrypt = new stream.Transform({ readableHighWaterMark: self.config.readingSpeed, decodeStrings: false });
                        decrypt._transform = async function(chunk, encoding, done) {
                            let decrypted = await self.file_protector.read(chunk);
                            done(null, decrypted);
                        };
