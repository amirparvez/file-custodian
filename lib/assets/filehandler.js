...
class FCFileHandler{
    ...
    async getS3FileSize(path){
        try{
            const headers = this.config.s3.headObject({ Key: path, Bucket: this.config.bucket_name }).promise();
            return headers.ContentLength ? headers.ContentLength : 0;
        }catch(error){ Config.debug ? console.log(error) : false; }
    ...
    async searchS3Files(dirPath, query){
        try{
            const files = await this.config.s3.listObjects({ Bucket: this.config.bucket_name, Delimiter: '/', Prefix: dirPath+(dirPath === "" ? "" : "/"), }).promise().then(data => {
            ...
            if(query !== null && typeof query === "string"){
                if(queryType === "extension"){
                    filteredFiles = files.filter(file => {
                        return path.extname(file.Key).replace(".", "").toLowerCase() === queryParam;
                    });
                }else{
                    if(queryType === "name"){
                        filteredFiles = files.filter(file => {
                            return file.Key.replace(path.extname(file.Key), "").toLowerCase() === queryParam;
                        });
                    }else{
                        if(queryType === "name_contains"){
                            filteredFiles = files.filter(file => {
                                return file.Key.replace(path.extname(file.Key), "").toLowerCase().includes(queryParam);
                            });
            ...
            for(let file of filteredFiles){
                const fileObj = await this.getS3File(file.Key); // Get data for each local file.
                if(fileObj){
    ...
    async searchLocalFiles(dirPath, query){
        ...
        if(isValid){
            ...
            if(isDir){
                ...
                let finalFiles = [];
                for(let file of filteredFiles){
                    const fileObj = await this.getLocalFile(dirPath+(dirPath === "" ? "" : "/")+file); // Get data for each local file.
                    if(fileObj){
                        finalFiles.push(fileObj);
                    }
                }
    ...
    async getAllFilesOfS3Directory(path){
        try{
            return await this.config.s3.listObjects({ Bucket: this.config.bucket_name, }).promise().then(data => {
                return data.Contents;
            }).catch(error => {
                Config.debug ? console.log(error) : false;
                return [];
