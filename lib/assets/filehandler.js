...
import FCMongoDBHandler from './mongodbhandler.js';
import FCMySQLHandler from './mysqlhandler.js';
...
class FCFileHandler{
    ...
    async newDatabase(options){
        if(options && typeof options === "object"){
            ...
            if(!validateConfig.success){ Config.debug ? console.log(`[FILEHANDLER:${this.config.type}] New database handler creation failed`) : false; }
            ...
            if(validateConfig.success){ Config.debug ? console.log(`[FILEHANDLER:${this.config.type}] New database handler created`) : false; return true; }
    ...
    async newProtector(options){
        if(options && typeof options === "object"){
            ...
            if(!validateConfig.success){ Config.debug ? console.log(`[FILEHANDLER:${this.config.type}] New file protector creation failed`) : false; return false; }

            this.file_protector = new FCFileProtector({ algorithm: validateConfig.object.algorithm,});

            if(validateConfig.success){ Config.debug ? console.log(`[FILEHANDLER:${this.config.type}] New file protector created`) : false; return true; }
