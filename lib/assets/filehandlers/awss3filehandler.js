...
class FCAWSS3FileHandler extends FileHandler{
    ...
    async copyFile(options){
        ...
        if(validateConfig.success){
            ...
            let response = await this.copyS3File(pathToFile, newPath);
            if(response){
                Logger.log(`${fsn} copied`);

                let file = await this.getS3File(newPath);
    ...
    async moveFile(options){
        ...
        if(validateConfig.success){
            ...
            let response = await this.copyS3File(fullPathToFile, fullNewPath);
            if(response){
                let response2 = await this.deleteS3File(fullPathToFile, []);
                if(response2 === true){
    ...
    // Private function.
    // Copies an s3 file.
    async copyS3File(fpath, newPath){
        const self = this;
        try{
            return await self.config.s3.copyObject({ Bucket: self.config.bucketName, CopySource: `${self.config.bucketName}/${fpath}`, Key: newPath, }).promise().then(() => { return true; }).catch((error) => { Logger.log(error); return false; })
        }catch(error){ Logger.log(error); }
    ...
    async renameS3File(fpath, newPath){
        const self = this;
        try{
            return await self.copyS3File(fpath, newPath).then(async (data) => {
                if(data === true){
                    try{
