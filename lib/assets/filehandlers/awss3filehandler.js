...
class FCAWSS3FileHandler extends FileHandler{
    ...
    async getFileSize(path, forceRequestToS3 = true){
        return await this.getS3FileSize(path, forceRequestToS3);
    }
    ...
    async setup(){
        ...
        if(this.config.s3){ return this.config.type === "bb-b2" ? true : await this.makeS3Bucket(this.config.basePath); }
        return true;
    }
    ...
    async makeS3File(path, contents, readStream, doEncrypt, isStream, contentLength, isReplacing = false){
        ...
        try{
            ...
            if(isStream === false){ contentStream = self.createDataReadStream(self.config.encryptingSpeed); }
            ...
            if(doEncrypt === true){
                ...
                const pipeline = pipelineWithoutPromise(contentStream, encryptStream, (error) => { Logger.log(error); });
    ...
    async deleteS3File(path){
        try{
            const allVersions = await this.getS3FileVersions(path);
