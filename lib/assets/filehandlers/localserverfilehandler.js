...
class FCLocalServerFileHandler extends FileHandler{
    ...
    async newFile(options){
        ...
        if(validateConfig.success){
            ...
            if(validateConfig.object.request){
                ...
                let newLocalFileCreated = await this.makeLocalFile(`${fullFolderPath}${fullFolderPath == "" ? "" : "/"}${validateConfig.object.name}.${validateConfig.object.ext}`, finalContents, validateConfig.object.isStream === true ? finalContents : null, doEncrypt, validateConfig.object.isStream, validateConfig.object.contentLength, false);
                if(newLocalFileCreated === true){
                    if(doEncrypt === true){ didEncrypt = true; }
    ...
    async deleteFile(options){
        ...
        if(validateConfig.success){
            ...
            Logger.log("LSfile deletion failed"); return false;
        }else{ Logger.log("LSfile deletion failed"); return false; }
    }
    ...
    async renameFile(options){
        ...
        if(validateConfig.success){
            ...
            Logger.log("LSfile renaming failed"); return false;
        }else{ Logger.log("LSfile renaming failed"); return false; }
    }
        ...
        try{
            ...
            if(doEncrypt === true){
                ...
                if(isReplacing){
                    ...
                    readStream.resume();
                    let writeStream = fs.createWriteStream(finalPath, {flags: 'a', highWaterMark: self.config.writingSpeed, });
                    writeStream.on('finish', function(){ console.log("finish"); });
                    writeStream.on('error', function(error){ console.log("error: ", error); });
                    await pipelineWithPromise(contentStream, writeStream).catch(error => { Logger.log(error); });
                }else{
