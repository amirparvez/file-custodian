    ...
    async newFile(options){
        ...
        if(validateConfig.success){
            ...
            if(validateConfig.object.request){
                ...
                let newMegaFileCreated = await this.makeMegaFile(`${fullFolderPath}${fullFolderPath === "" ? "" : "/"}${validateConfig.object.name}.${validateConfig.object.ext}`, finalContents, validateConfig.object.isStream === true ? finalContents : null, doEncrypt, validateConfig.object.isStream, validateConfig.object.contentLength, false);
                if(newMegaFileCreated === true){
                    if(doEncrypt === true){ didEncrypt = true; }
            ...
            if(exists){
                try{
                    ...
                    if(doEncrypt === true){
                        ...
                        helperStream._transform = async function(chunk, encoding, callback){ this.push(chunk); callback(); };
                        ...
                        helperStream.push(Buffer.from(`(${uniqueIV})`)); // contentStream.push throws error: contentStream.push is not a function, contentStream is stream returned from megaFile.download() in getMegaFileContents
