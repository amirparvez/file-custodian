...
class FCMegaFileHandler extends FileHandler{
        ...
        try{
            ...
            if(validateConfig.success){
                ...
                if(validateConfig.object.id !== undefined && validateConfig.object.id !== null){
                    ...
                    folder = validateConfig.object.name && validateConfig.object.ext ? pathToFile.replace(`${validateConfig.object.name}.${validateConfig.object.ext}`, "") : pathToFile;
                }

                let file = await this.getMegaFile(fullPathToFile);
                if(file){
                    folder = folder.includes(file.name) ? folder.replace(file.name, "") : folder;

                    let ext = path.extname(file.name).replace(".", "").toLowerCase();
                    return await self.newFCFile({ name: file.name.replace("."+ext, ""), ext, folder, handler: self, isEncrypted: false, });
                }
            }
    ...
    async copyFile(options){
        ...
        if(validateConfig.success){
            ...
            await this.makeMegaDir(path.dirname(newPath), 0, "root");

            let response = await this.copyMegaFile(pathToFile, newPath, validateConfig.object.isEncrypted);
            if(response){
                Logger.log(`${fsn} copied`);

                let file = await this.getMegaFile(newPath);
                if(file){
                    let ext = path.extname(file.name).replace(".", "").toLowerCase();
                    return await self.newFCFile({ name: file.name.replace("."+ext, ""), ext: ext, folder: newPath.replace(file.name, ""), handler: self, isEncrypted: validateConfig.object.isEncrypted, });
                }
    ...
    async moveFile(options){
        ...
        if(validateConfig.success){
            ...
            await this.makeMegaDir(path.dirname(fullNewPath), 0, "root");

            let response = await this.copyMegaFile(fullPathToFile, fullNewPath);
            if(response){
                let response2 = await this.deleteMegaFile(fullPathToFile);
                if(response2 === true){
                    ...
                    if(this.database_handler){
                        ...
                        if(exists !== null && exists !== undefined){
                            ...
                            if(response3 === true){
                                let file = await this.getMegaFile(fullNewPath);
                                if(file){
                                    let ext = path.extname(file.name).replace(".", "").toLowerCase();
                                    return await self.newFCFile({ name: file.name.replace("."+ext, ""), ext: ext, folder: newPath.replace(file.name, ""), handler: self, isEncrypted: validateConfig.object.isEncrypted, });
                                }
    ...
    // Private function.
    // Copies a mega file.
    async copyMegaFile(fpath, newPath, doEncrypt = false){
        const self = this;
        try{
            let megaFile = await self.getMegaFile(fpath);
            if(megaFile){
                let megaFileCheck = await self.getMegaFile(newPath);
                if(!megaFileCheck){
                    const readStream = megaFile.download({ initialChunkSize: self.config.encryptingSpeed, maxChunkSize: self.config.encryptingSpeed, });
                    readStream.pause();
                    //readStream.on('end', function(){ readStream.destroy(); }); // Destroying stream on end might sometimes cause troubles encrypting-decrypting the last chunk.
                    readStream.on('error', function(error){ readStream.destroy(); });

                    let finalContents = readStream;
                    let isStream = true;
                    let contentLength = megaFile.size;

                    let newMegaFileCreated = await self.makeMegaFile(newPath, finalContents, readStream, doEncrypt, isStream, contentLength, false);
                    return newMegaFileCreated;
                }
            }
        }catch(error){ Logger.log(error); }
    ...
    async searchMegaFiles(dirPath, query, forceRequestToProvider = false){
        try{
            var files = [];
    ...
    async getAllFilesOfMegaDirectory(path, forceRequestToProvider = false){
        try{
            if(this.database_handler !== null && forceRequestToProvider === false){
                // If database is connected, search in the database.
