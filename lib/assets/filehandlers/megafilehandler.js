        ...
        if(didMakeDir === true){
            ...
            if(exists){
                try{
                    ...
                    if(isStream === false){ contentStream = self.createDataReadStream(Buffer.concat([Buffer.from(`(${uniqueIV})`), Buffer.isBuffer(contents) ? contents : Buffer.from(contents)]), self.config.encryptingSpeed); }
                    ...
                    if(doEncrypt === true){
                        ...
                        // contentLength does not needs to be increased for encryptStream by 34 for IV since uniqueIV is not pushed into contentStream instead it's pushed into helperStream...
                        // ...due to which encrypt stream will only need to the length of contents without IV length.
                        const encryptStream = self.createEncryptStream(self.config.encryptingSpeed, contentStream, contentLength+34, uniqueIV);
                        ...
                        //helperStream.push(Buffer.from(`(${uniqueIV})`)); // contentStream.push throws error: contentStream.push is not a function, when doEncrypt is true and a mega file is being replaced (being protected) then contentStream is stream returned from megaFile.download() in getMegaFileContents, probably a delay will work.
                        //contentStream.push(Buffer.from(`(${uniqueIV})`));
                        contentStream.resume();

                        // Data should be pushed to contentStream always in flowing state.
                        //isStream === false ? contentStream.push(contents) : false;
                        //isStream === false ? contentStream.push(null) : false; // Required to end stream.Readable after all data has been pushed
                    ...
                    if(isReplacing === true){
                        // Wait for some time to gurantee file contents update otherwise readFileIV will not be able to get the file IV since...
                        // ...it will read the file contents of old un encrypted file due to early function call in case of encryption-replacing.
