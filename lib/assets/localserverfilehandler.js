...
class FCLocalServerFileHandler extends FileHandler{
        ...
        if(validateConfig.success){
            ...
            if(validateConfig.object.request){
                ...
                for(let requestFile of requestFiles){
                    let { contents, contentType, contentLength, readStream } = await this.getLocalFileContents(requestFile.path, false); // contents & readStream are same since 2nd parameter, decrypt, is false. If decrypt is true, contents is a pipeline.
                    let isStream = true;
                    let doEncrypt = false;
                    ...
                    if(contents){
                        ...
                        let newLocalFileCreated = await this.makeLocalFile(`${self.config.base_path}${self.config.base_path === "" ? "" : "/"}${fileFolder}${fileFolder === "" ? "" : "/"}${requestFileName}`, finalContents, readStream, doEncrypt, isStream, contentLength, false);
    ...
    async searchFiles(options){
        ...
        if(validateConfig.success){
            ...
            let files = await this.searchLocalFiles(pathToFile.toString().trim() === "*" ? "*" : `${self.config.base_path}${self.config.base_path === "" ? "" : "/"}${pathToFile}`, validateConfig.object.query);
            if(files){
                let FCFiles = [];
    ...
    async syncDatabase(){
        try{
            let files = await this.getAllFilesOfLocalDirectoryRecursively(this.config.base_path);
            console.log(files);
    ...
    async replaceFileContents(options){
        ...
        try{
            if(validateConfig.success){
                ...
                return await this.makeLocalFile(`${this.config.base_path}${this.config.base_path === "" ? "" : "/"}${filePath}`, validateConfig.object.new_contents, validateConfig.object.readStream, validateConfig.object.doEncrypt, validateConfig.object.isStream, validateConfig.object.contentLength, true);
            }else{ return false; }
        }catch(error){
            return false;
        }
    ...
    async getFileContents(file){
        try{
            ...
            return await this.getLocalFileContents(`${this.config.base_path}${this.config.base_path === "" ? "" : "/"}${filePath}`, file.config.isEncrypted);
        }catch(error){
    ...
    async makeLocalFile(path, contents, readStream, doEncrypt, isStream, contentLength, isReplacing = false){
        ...
        try{
            ...
            let finalPath = isReplacing === true ? `${self.config.base_path}/temp/temp-${random}.temp` : `${path}`;

            if(isReplacing){ await self.makeLocalDir(`${self.config.base_path}/temp`); }

            if(doEncrypt === true){
                let encryptStream = self.createEncryptStream(self.config.encryptingSpeed, contentStream, contentLength);

                contentStream.resume();
                await pipelineWithPromise(contentStream, encryptStream, fs.createWriteStream(finalPath, {flags: 'a', highWaterMark: self.config.encryptingSpeed, })).catch(error => { Config.debug ? console.log(error) : false; });
            }else{
                if(isReplacing){
