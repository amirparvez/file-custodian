...
class FCLocalServerFileHandler extends FileHandler{
    ...
    async replaceFileData(options){
        var validateConfig = await ObjectValidator.validate({ object: options, against: "Func_ReplaceFileData", });
        try{
            if(validateConfig.success){
                var beautifiedPath = await validateConfig.object.file.beautifyPath(validateConfig.object.file.config.folder);
                var filePath = `${beautifiedPath}${beautifiedPath == "/" ? "" : "/"}${validateConfig.object.file.config.name}.${validateConfig.object.file.config.ext}`;
