...
class FCLocalServerFileHandler extends FileHandler{
        ...
        if(validateConfig.success){
            ...
            if(validateConfig.object.request){
                ...
                for(let requestFile of requestFiles){
                    let { contents, contentType, isStream, readStream } = await this.getLocalFileContents(requestFile.path, false); // contents & readStream are same since 2nd parameter, decrypt, is false. If decrypt is true, contents is a pipeline.
                    let doEncrypt = false;
                    let didEncrypt = false;

                    if(contents){
                        let finalData = contents;
                        if(this.file_protector !== null && validateConfig.object.isEncrypted === false){
                            ...
                            doEncrypt = true;
                            if(isStream === false){
                                finalData = await this.file_protector.protect(contents);
                                if((finalData.toString() === contents.toString()) || finalData == null){ didEncrypt = false; }else{ didEncrypt = true; }
                            }else{
                                if(isStream === true){
                        ...
                        let newLocalFileCreated = await this.makeLocalFile(`${self.config.base_path}${fileFolder}/${requestFileName}`, finalData, doEncrypt, isStream);
                ...
                if(this.file_protector !== null && validateConfig.object.isEncrypted === false){
                    // If a file protector is assigned & passed value of isEncrypted is not true, encrypt the data.
                    doEncrypt = true;
                    if(validateConfig.object.isStream === false){
                        ...
                        if(validateConfig.object.isStream === true){
                            // Encrypt contents while writing the file.
                        }
                    }
                }

                let newLocalFileCreated = await this.makeLocalFile(`${self.config.base_path}${fileFolder}/${validateConfig.object.name}.${validateConfig.object.ext}`, finalData, doEncrypt, validateConfig.object.isStream);
                if(newLocalFileCreated === true){
                    if(doEncrypt === true){ didEncrypt = true; }
    ...
    async getFileContents(file){
        try{
            ...
            return await this.getLocalFileContents(`${this.config.base_path}${filePath}`, file.config.isEncrypted);
        }catch(error){
