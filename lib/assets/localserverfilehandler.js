    ...
    async replaceFileContents(options){
        ...
        try{
            if(validateConfig.success){
                ...
                return await this.makeLocalFile(`${this.config.base_path}${this.config.base_path === "" ? "" : "/"}${filePath}`, validateConfig.object.new_contents, validateConfig.object.doEncrypt, validateConfig.object.isStream);
            }else{ return false; }
        }catch(error){
            return false;
        }
