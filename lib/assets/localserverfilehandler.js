            ...
            if(validateConfig.object.request){
                ...
                for(let requestFile of requestFiles){
                    ...
                    if(contents){
                        ...
                        let newLocalFileCreated = await this.makeLocalFile(`${self.config.base_path}${self.config.base_path === "" ? "" : "/"}${fileFolder}${fileFolder === "" ? "" : "/"}${requestFileName}`, finalContents, doEncrypt, isStream, contentLength);
                ...
                let newLocalFileCreated = await this.makeLocalFile(`${self.config.base_path}${self.config.base_path === "" ? "" : "/"}${fileFolder}${fileFolder === "" ? "" : "/"}${validateConfig.object.name}.${validateConfig.object.ext}`, finalContents, doEncrypt, validateConfig.object.isStream, validateConfig.object.contentLength);
                if(newLocalFileCreated === true){
                    if(doEncrypt === true){ didEncrypt = true; }
    ...
    async replaceFileContents(options){
        ...
        try{
            if(validateConfig.success){
                ...
                return await this.makeLocalFile(`${this.config.base_path}${this.config.base_path === "" ? "" : "/"}${filePath}`, validateConfig.object.new_contents, validateConfig.object.doEncrypt, validateConfig.object.isStream, validateConfig.object.contentLength);
            }else{ return false; }
        }catch(error){
            return false;
        }
