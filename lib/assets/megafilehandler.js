    ...
    async getMegaDir(path, folderToGetIn = "root"){
        ...
        try{
            ...
            for(let folderToGet of allFolders){
                //if(i === allFolders.length){ break; }
                if(!currentFolder.children){ break; } // if no files in currentFolder
                if(folderToGet.toString().trim() === ""){ continue; } // if folder of name "", in case of last folder sometimes
        ...
        if(path && path.toString().trim() !== ""){
            ...
            if(exists === false){
                try{
                    ...
                    let i = startIndex;
                    let lastFolderFound = false; // whether lastFolderThatExists in path supplied is processed
                    ...
                    let folderToCreate = allFolders[i]; // folder which should be created, identified using array index from path split array
                    let lastFolderIndex = lastFolderThatExists ? allFolders.findIndex(x => { return x.toString().trim() === lastFolderThatExists.name.toString().trim(); }) : null; // index of lastFolderThatExists in path split array, this keeps increasing as new folders are created
                    if(allFolders.length > 1){
                        // Multiple folder creation
                        // example path: "folder/folder1/folder2/folder3"
                        let thisFolderIndex = allFolders.findIndex(x => { return x.toString().trim() === folderToCreate.toString().trim(); }); // index of folder being created in path split array
                        thisFolderIndex = thisFolderIndex < 0 ? 0 : thisFolderIndex; // this can never happen

                        let conditionOne = folderToCreate.toString().trim() === lastFolderThatExists.name.toString().trim(); // folder which is being created is the same folder which is lastFolderThatExists, it already exists since getMegaDir returns last folder that exists in the path, therefore it already exists
                        ...
                        //console.log("~ ", `${lastFolderThatExists.name.toString().trim()}(${lastFolderIndex})`, `${folderToCreate}(${thisFolderIndex})`, isThisFolderAlreadyCreated);
    ...
    async searchMegaFiles(dirPath, query, forceRequestToMega = false){
        try{
            ...
            if(this.database_handler !== null && forceRequestToMega === false){
                // At large scale, making a request to mega every time might affect the cost.
