...
class ObjectValidator {
        ...
        this.schemas = [
            ...
            },
            {
                ...
                schema: function(){
                    return Joi.object({
                        name: Joi.string().required(),
                        type: Joi.string().valid('local-server', 'aws-s3', 'do-spaces', 'bb-b2', 'mega').required(),
                        basePath: Joi.string().optional().allow("").default(""),
                        bucketName: Joi.string().optional().when("type", {
                            is: Joi.valid('aws-s3', 'do-spaces', 'bb-b2'),
                            then: Joi.required(),
                        }),
                        ...
                        endpoint: Joi.string().optional().when("type", {
                            is: Joi.valid("do-spaces", "bb-b2"),
                            then: Joi.required(),
                        }),
            ...
            },
            {
                ...
                schema: function(){
                    return Joi.object({
                        ...
                        readStream: Joi.optional().default(null),
            ...
            },
            {
                ...
                schema:function(){
                    return Joi.object({
                        path: Joi.string(),
                        newPath: Joi.string(),
                        folderName: Joi.string().when("newPath", {
                            not: Joi.exist(),
                            then: Joi.required(),
                        }),
                        name: Joi.string().when("path", {
            ...
            },
            {
                ...
                schema: function(){
                    return Joi.object({
                        ...
                        forceRequestToProvider: Joi.boolean().default(false),
                    }).options({ stripUnknown: true });
                },
            },
            {
    ...
    async validate(options){
        if(options && typeof options == "object" && options.object && options.against){
            ...
            }catch(error){ Logger.log(`VALIDATION ERROR: ${options.against}`); Logger.error("ov_io"); return {success: false, object: null,}; }
            ...
            if(response.error === undefined){
                ...
                if(schemaToCheckAgainst){
                    ...
                    }catch(error){ Logger.log(`VALIDATION ERROR: ${options.against}`); Logger.error("ov_vf", options.against.toString() || ""); return {success: false, object: null,}; }
                }else{ Logger.log(`VALIDATION ERROR: ${options.against}`); Logger.error("ov_isn"); return {success: false, object: null,}; }
            }
        }else { Logger.log(`VALIDATION ERROR: ${options.against}`); Logger.error("ov_io"); return {success: false, object: null,}; }
    }
}
