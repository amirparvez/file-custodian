...
class FCMySQLHandler{
    ...
    async getAllModels(options){
        ...
        if(validateConfig.success){
            if(validateConfig.object.user_id !== undefined && validateConfig.object.user_id !== null){
                if(validateConfig.object.folder !== undefined && validateConfig.object.folder !== null){
    ...
    async getModel(options){
        if(typeof options === "object" && options.path && typeof options.path === "string"){
            if(options.user_id !== undefined && options.user_id !== null){
                return this.FileModel.findOne({ where: { path: options.path, user_id: options.user_id }, include: {all: true,}, });
            }else{
                return this.FileModel.findOne({ where: { path: options.path, }, include: {all: true,}, });
            }
        }else{ return null; }
    }
